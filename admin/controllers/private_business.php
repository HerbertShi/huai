<?php
class Private_business extends Bank_Controller {
	...
	function save_list() {
		...
		$config['base_url'] = site_url('/private_business/save_list/');
		...
		$this->load->view('/private_business/save_list', $data);
	}
	...
	function lend_list() {
		...
		$this->load->helper('url');
		$this->load->library('pagination');
		$config['base_url'] = site_url('/private_business/lend_list/');
		$config['total_rows'] = $total_num;
		$config['per_page'] = 20; 
		$config['first_link'] = '首页'; // 第一页显示   
		$config['last_link'] = '末页'; // 最后一页显示   
		...
		$this->load->view('/index/Public_header');
		$this->load->view('/private_business/lend_list', $data);
	}
	...
	function save_info() {
		...
		if ($obj_id != '') {
			...
			$this->load->helper('url');
			$this->load->view('/private_business/save_info',$data);
		}
	}
	...
	function lend_info() {
		...
		if ($obj_id != '') {
			...
			$data['obj_info'] = $obj_info;
			$this->load->helper('url');
			$this->load->view('/private_business/lend_info',$data);
		}
	}
	...
	function add_save() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->helper('url');
			$this->load->view('/private_business/add_save',$data);
		} else {
			$this->load->database();
			$this->db->set('title', htmlspecialchars(trim($title),ENT_QUOTES));
			$this->db->set('add_time', time());
			$this->db->set('content', $content);
			$this->db->insert('private_save_business');
			$this->load->helper('url');
			redirect('/private_business/save_list');
		}
	}
	...
	function add_lend() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['content'] = $content;
			$this->load->helper('url');
			$this->load->view('/private_business/add_lend',$data);
		} else {
			$this->load->database();
			$this->db->set('title', htmlspecialchars(trim($title),ENT_QUOTES));
			$this->db->set('add_time', time());
			$this->db->set('content', $content);
			$this->db->insert('private_lend_business');
			$this->load->helper('url');
			redirect('/private_business/lend_list');
		}
	}
	...
	function update_save() {
		...
		if ($obj_id != "") {
			...
			if ($this->form_validation->run()) {
				...
				$this->db->update('private_save_business');
				$this->load->helper('url');
				redirect('/private_business/save_list/');
			}
		}  else {
			...
			$this->load->helper('url');
			$this->load->view('/private_business/update_save',$data);
		}
	}
	...
	function update_lend() {
		...
		if ($obj_id != "") {
			...
			if ($this->form_validation->run()) {
				...
				$this->db->set('update_time', time());
				$this->db->where('id', $obj_id);
				$this->db->update('private_lend_business');
				$this->load->helper('url');
				redirect('/private_business/lend_list/');
			} 
		} else {
			...
			$data['item_info'] = $item_info;
			$this->load->helper('url');
			$this->load->view('/private_business/update_lend',$data);
		}
	}
	...
	public function delete_save(){
		...
		if (!empty($ids_array)) {
			...
			$this->load->helper('url');
			redirect('/private_business/save_list', 'refresh');
		}
	}
	...
	public function delete_lend(){
		...
		if (!empty($ids_array)) {
			...
			$this->load->helper('url');
			redirect('/private_business/lend_list', 'refresh');
		}
	}
