<?php
class Notice extends Bank_Controller {
	...
	function view_list() {
		...
		$this->load->database();
		$sql = "SELECT * FROM notice WHERE ".$where." ORDER BY id desc";
		...
		$config['base_url'] = site_url('/notice/view_list/');
		...
		$this->load->view('/notice/view_list', $data);
	}
	...
	function item_info() {
		...
		if ($obj_id != '') {
			$this->load->database();
			$sql = "SELECT * FROM notice where id=".$obj_id;
			$query = $this->db->query($sql);
			$obj_info = $query->row_array();
			$data['obj_info'] = $obj_info;
			$this->load->helper('url');
			$this->load->view('/notice/item_info',$data);
		}
	}
	...
	public function delete(){
		...
		if (!empty($ids_array)) {
			...
			$this->load->helper('url');
			redirect('/notice/view_list', 'refresh');
		}
	}
}
