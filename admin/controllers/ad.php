<?php
class Ad extends Bank_Controller {
	...
	function view_list() {
		...
		$this->load->database();
		$sql = "SELECT * FROM ad where ".$where." ORDER BY id desc";
		...
		$config['base_url'] = site_url('/ad/view_list/');
		...
		$this->load->view('/ad/view_list', $data);
	}
	...
	function ad_info() {
		...
		if ($obj_id != '') {
			$this->load->database();
			$sql = "SELECT * FROM ad where id=".$obj_id;
			$query = $this->db->query($sql);
			$obj_info = $query->row_array();
			$data['obj_info'] = $obj_info;
			$this->load->helper('url');
			$this->load->view('/ad/info',$data);
		}
	}
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($_FILES['upload_img']['name'] != '') {
				...
				if (!$this->upload->do_upload('upload_img')) {
			   		...
			   		$this->load->view('/ad/add', $error);
			  	} else{
			   		$upload_data = $this->upload->data();
				   	$img = '/uploads/Ad/'.$upload_data['file_name'];
			  	}
			}
