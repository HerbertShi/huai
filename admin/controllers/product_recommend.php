<?php
class Product_recommend extends Bank_Controller {
	...
	function add() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->helper('url');
			$this->load->view('/product_recommend/add',$data);
		} else {
			$this->load->database();
			$this->db->set('title', htmlspecialchars(trim($title),ENT_QUOTES));
			$this->db->set('add_time', time());
			$this->db->set('content', $content);
			$this->db->insert('product_recommend');
			$this->load->helper('url');
			redirect('/product_recommend/view_list');
		}
	}
	...
	function update() {
		...
		if ($obj_id != "") {
			...
			if ($this->form_validation->run()) {
				...
				$this->db->update('product_recommend');
				$this->load->helper('url');
				redirect('/product_recommend/view_list/');
			} 
		} else {
			...
			$this->load->helper('url');
			$this->load->view('/product_recommend/update',$data);
		}
	}
	public function delete(){
		...
		if (!empty($ids_array)) {
			...
			$this->load->helper('url');
			redirect('/product_recommend/view_list', 'refresh');
		}
	}
