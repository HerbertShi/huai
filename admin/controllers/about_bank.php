<?php
class About_bank extends Bank_Controller {
	...
	function view_list() {
		...
		$this->load->database();
		$sql = "SELECT * FROM about_bank WHERE ".$where." ORDER BY id desc";
		...
		$data['aboutbank_list'] = $aboutbank_list;
		$this->load->view('/index/Public_header');
		$this->load->view('/about_bank/view_list', $data);
	}
	...
	function item_info() {
		...
		if ($obj_id != '') {
			$this->load->database();
			$sql = "SELECT * FROM about_bank where id=".$obj_id;
			$query = $this->db->query($sql);
			$obj_info = $query->row_array();
			$data['obj_info'] = $obj_info;
			$this->load->helper('url');
			$this->load->view('/about_bank/item_info',$data);
		}
	}
	...
	function add() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->helper('url');
			$this->load->view('/about_bank/add',$data);
		} else {
			$this->load->database();
			$this->db->set('title', htmlspecialchars(trim($title),ENT_QUOTES));
			$this->db->set('add_time', time());
			$this->db->set('content', $content);
			$this->db->insert('about_bank');
			$this->load->helper('url');
			redirect('/about_bank/view_list');
		}
	}
	...
	function update() {
		...
		if ($obj_id != "") {
			...
			$this->load->helper('url');
			$this->load->view('/about_bank/update',$data);
		}
	}
	...
	public function delete(){
		...
		if (!empty($ids_array)) {
			...
			$this->load->helper('url');
			redirect('/about_bank/view_list', 'refresh');
		}
	}
